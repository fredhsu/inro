...
- `--document-key KEY` create a stable keyed Document; use for recurring reports
...
Diagnostics features being added for this helper: `--dry-run` to show resolved server/format/title without sending, and `--json` for machine-readable output.
...
A doctor script is being added for fast environment checks:

```bash
node .agents/skills/inro-preview/scripts/inro-doctor.mjs --server https://inro.example.com --json
```

Expected checks: config discovery, token source without revealing token, server reachability, auth result, API base path, and whether returned URLs are usable from the configured server URL.

The send helper is also gaining:

```bash
node .agents/skills/inro-preview/scripts/send-inro-document.mjs ./document.md --dry-run --json
```

Use `--dry-run` before sending from a new machine or cron job; use `--json` when another agent/script needs to parse `documentId`, `revisionId`, `latestUrl`, or `revisionUrl`.
