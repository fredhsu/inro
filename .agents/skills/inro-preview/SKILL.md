...
By default, do **not** pass `--source-agent`: let the helper resolve it from `INRO_SOURCE_AGENT` or `~/.inro/client-config.json`. Only pass `--source-agent ...` when the user explicitly requests a one-off different identity.

Source-agent precedence: `--source-agent`, `INRO_SOURCE_AGENT`, `~/.inro/client-config.json`, then `llm-agent`. If the UI shows an unexpected source such as `hermes-telegram`, diagnose CLI/env overrides before changing config.
...
For Markdown, use a `.md` file. For HTML, use `.html`; the CLI infers the format. Prefer the bundled helper for normal hosted sends because it reads `~/.inro/client-config.json`; use `--source-agent ...` here only for a deliberate one-off override.
...
15 7 * * * cd /home/fred/project && ./scripts/generate-digest.sh && INRO_SOURCE_AGENT=cron-digest-agent node .agents/skills/inro-preview/scripts/send-inro-document.mjs .scratch/daily-digest.md --document-id "$(cat .scratch/daily-digest.document-id)" --revision-summary "Daily digest $(date -I)" --idempotency-key "daily-digest-$(date -I)" >> .scratch/inro-cron.log 2>&1
```

For background agents, prefer stable identity from `INRO_SOURCE_AGENT` or `~/.inro/client-config.json` (`--document-key` at creation, persisted `--document-id` for appends) plus `--idempotency-key` so retries do not create duplicate Revisions. Use `--source-agent` only for an intentional one-off override.
